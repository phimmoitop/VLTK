...
function updateMonsters() {
  const updateRadiusSq = 2000 * 2000; // Tăng radius update
  const px = playerX, py = playerY;

  for (const m of monsters) {
    // ✅ CHỈ UPDATE GẦN PLAYER
    ...
    if (m.state === 'Stand') {
      ...
      // ✅ FIX: DÙNG SQUARED DISTANCE (KHÔNG sqrt)
      const distFromBaseSq = (m.x - m.baseX) ** 2 + (m.y - m.baseY) ** 2;
      if (distFromBaseSq > 60 * 60 || m.actionTimer <= 0) {
        m.state = 'Stand';
...
function drawMonsters(ctx, camX, camY, canvas) {
  ...
  for (const m of monsters) {
    // ✅ CULLING 1: WORLD SPACE (nhanh)
    if (!isInViewMonster(m.x, m.y, camX, camY, canvas)) continue;
    ...
    // ✅ CULLING 2: SCREEN SPACE (chính xác)
    if (sx + f.w < 0 || sy + f.h < 0 || sx > canvas.width || sy > canvas.height) continue;

    // VẼ GIỮ NGUYÊN (shadow, name, HP, sprite)
