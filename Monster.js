...
// Danh sách quái, thông tin, hướng
const monsters = [];
...
// Cập nhật trạng thái di chuyển & animation quái
function updateMonsters() {
  for (const m of monsters) {
    ...
    if (m.state === 'Stand') {
      ...
      const distFromBase = Math.hypot(m.x - m.baseX, m.y - m.baseY);
      if (distFromBase > 60 || m.actionTimer <= 0) {
        m.state = 'Stand';
      ...
      if (m.x < EDGE_MARGIN || m.x > mapWidth - EDGE_MARGIN || m.y < EDGE_MARGIN || m.y > mapHeight - EDGE_MARGIN) {
